...
class DbBackup {
  ...
  static void notifyNeedsBackup(Collection collection) {
    ...
    final diff = lastBackupTime == null ? null : DateTime.now().difference(lastBackupTime);
    final awaitTime = diff == null ? Duration.zero : maxTimeBetweenBackups - diff;
  ...
  static Future<void> _createBackup(Collection collection) async {
    ...
    final filePath = p.join(
      dirPath,
      'database_backup_${datetimeFormat.format(now)}.db',
    );
    await db.execute('''
      ...
      try {
        if (file is File) {
          if (file.nameWithoutExtension.startsWith('database_backup_')) {
            final dateString = file.nameWithoutExtension.substring(
              'database_backup_'.length,
            );
        ...
        log(
          LgLvl.error,
          'Error while cleaning up db backup files',
          e: e,
          st: st,
        );
      }
