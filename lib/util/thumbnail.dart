...
abstract class ThumbnailUtils {
  static Future<Directory?> ensureThumbnailFolderCreated(
  ...
  static Future<File?> ensureThumbnailCreated(Item item) async {
    final thumbnailsFolder = await ensureThumbnailFolderCreated(
      item.collection,
    );
  ...
  static Future<File?> createThumbnail(Item item, {File? thumbnail}) async {
    if (thumbnail == null) {
    ...
    if (result.exitCode != 0) {
      log(
        LgLvl.error,
        'Error while creating thumbnail for: $itemPath\n${result.stderr}',
      );
      return null;
    }
    return thumbnail;
