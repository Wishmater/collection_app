...
void main() async {
  ...
  log(LgLvl.info, 'SUCCESS !!!', type: LgType.script);
...
Future<void> importPrnhb({bool clearDb = true}) async {
  ...
  await DbHelper.waitForAllDbOperationsToFinish();
  collectionService.addCollection(collection, checkIfAlreadyExists: false);
  await DbHelper.waitForAllDbOperationsToFinish();
  final rootTagCreator = tagService.getTagByNameOrCreate(
    Tag(added: addedDatetime, name: 'Creator'),
    collection,
  );
  final rootTagContent = tagService.getTagByNameOrCreate(
    Tag(added: addedDatetime, name: 'Content Tag'),
    collection,
  );
  ...
  for (final entry in addToPathsAtTheEnd.entries) {
    final item = collection.items.firstOrNullWhere(
      (e) => e.getAbsoluteFilePath() == entry.key,
    );
    if (item == null) {
      log(
        LgLvl.warning,
      ...
      if (entry.value.priority != null &&
          (item.explorePriority == null || item.explorePriority! > entry.value.priority!)) {
  ...
  for (final child in children) {
    ...
    if (child is Directory) {
      ...
      if (parentName == '!channels') {
        ...
        for (final char in childName.characters) {
          if (insideParenthesis) {
            if (char == ')' || char == ',') {
              ...
              final newTag = tagService.getTagByNameOrCreate(
                Tag(name: newTagName, parentTag: rootTagContent),
                collection,
              );
        ...
        final creator = tagService.getTagByNameOrCreate(
          Tag(
            added: addedDatetime,
            name: creatorName,
        ...
        if (childName == 'rated' || childName == '!channels') {
          ...
          addedTag = tagService.getTagByNameOrCreate(
            Tag(name: childName, parentTag: rootTagContent),
            collection,
          );
        }
      }
      await _processFolder(
        child,
        [...tags, if (addedTag != null) addedTag],
        addedDatetime: addedDatetime,
        collection: collection,
        rootTagCreator: rootTagCreator,
        rootTagContent: rootTagContent,
      ...
      if (childPriority != null) {
        addToPathsAtTheEnd[resolvedShortcutPath]!.priority = min(
          childPriority,
          addToPathsAtTheEnd[resolvedShortcutPath]!.priority ?? 10,
        );
      }
      for (final e in tags) {
      ...
      final childRelativePath = p.relative(
        childAbsolutePath,
        from: collection.baseDirectory,
      );
      var name = childName;
      int? rating;
      ...
      itemService.addItem(newItem, checkIfAlreadyExists: false);
    }
  }
}
